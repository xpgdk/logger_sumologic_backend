defmodule LoggerSumologicBackend.Clients.HTTPotion do
  ...
  def init(config) do
    ...
    %{
      endpoint: endpoint,
      check_hostname: get_endpoint_hostname(endpoint),
  ...
  def log_event(config, entries, retry) do
    case HTTPotion.post(config.endpoint,
                        body: generate_body(config, entries),
                        headers: config.headers,
                        ibrowse: ibrowse_options(config) ) do
  ...
  defp ssl_verify_options(hostname) do
    [
  ...
  defp ssl_options(config) do
    Keyword.merge(default_ssl_options(), ssl_verify_options(config.check_hostname))
  end

  defp ibrowse_options(config) do
    [
      ssl_options: ssl_options(config)
    ]
  end
  ...
  defp get_endpoint_hostname(endpoint) do
    uri = URI.parse(endpoint)
    String.to_char_list(uri.host)
  end
  ...
  defp add_header(header_list, name, value) do
    Keyword.put(header_list, name, value)
  end
